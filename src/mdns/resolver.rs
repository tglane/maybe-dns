...
use crate::dns::{Packet, Question, QClass, QType};
use super::Response;
...
impl Resolver {
    pub fn new() -> Self {
        ...
        Self {
            sock: Arc::new(builder.bind(format!("0.0.0.0:{}", QUERY_PORT)).expect("[ERROR] Socket binding failed")),
    ...
    pub fn start_discover(&mut self, record_name: &str, duration: Duration) {
        ...
        let handle = std::thread::spawn(move || {
            socket.set_read_timeout(Some(duration)).expect("[ERROR] Socket configuration failed");
            ...
            socket.send_to(&dns_query.to_bytes(), format!("{}:{}", QUERY_IP, QUERY_PORT)).unwrap();
                ...
                match socket.recv_from(&mut buffer) {
                    Ok((size, peer)) => {
                        match Packet::try_from(&buffer[..size]) {
                            Ok(packet) => {
                                if packet.header.id != packet_id {
                                    match responses.lock() {
                                        Ok(mut guarded_access) => guarded_access.push_back(Response::with(peer, Ok(packet))),
                                        Err(_) => (),
                                    }
                                }
                            },
                            Err(err) => {
                                match responses.lock() {
                                    Ok(mut guarded_access) => guarded_access.push_back(Response::with(peer, Err(err))),
                                    Err(_) => (),
                                }
                            },
                        }
