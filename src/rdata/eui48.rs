use std::convert::TryFrom;

use crate::buffer::DnsBuffer;
use crate::byteconvertible::ByteConvertible;
use crate::error::DnsError;
use crate::rdata::{RData, RecordData, RecordType};

/// The EUI48 resource record (RR) is used to store a single EUI-48
/// address in the DNS.
#[derive(Clone, Debug, Default, PartialEq, Eq)]
pub struct Eui48([u8; 6]);

impl<'a> TryFrom<&mut DnsBuffer<'a>> for Eui48 {
    type Error = DnsError;

    fn try_from(buffer: &mut DnsBuffer<'a>) -> Result<Self, Self::Error> {
        Ok(Self(buffer.extract_bytes(6)?.try_into()?))
    }
}

impl RData for Eui48 {
    fn record_type(&self) -> RecordType {
        RecordType::EUI48
    }

    fn into_record_data(self) -> RecordData {
        RecordData::EUI48(self)
    }
}

impl ByteConvertible for Eui48 {
    fn byte_size(&self) -> usize {
        6
    }

    fn to_bytes(&self) -> Vec<u8> {
        self.0.to_vec()
    }
}
